...
int rt_hw_usart_init(void)
{
    ...
    for (int i = 0; i < obj_num; i++)
    {
        ...
        /* register UART device */
			 if(uart_obj[i].config->Instance==USART6)	uart_obj[i].serial.config.baud_rate=BAUD_RATE_19200;
			 if(uart_obj[i].config->Instance==USART2)	uart_obj[i].serial.config.baud_rate=BAUD_RATE_9600;
			 if(uart_obj[i].config->Instance==UART5)	uart_obj[i].serial.config.baud_rate=BAUD_RATE_38400;

			 //if(uart_obj[i].config->Instance!=USART1)	uart_obj[i].serial.config.baud_rate=BAUD_RATE_230400;
