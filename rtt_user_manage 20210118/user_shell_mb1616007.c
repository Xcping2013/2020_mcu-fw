...
uint8_t MB1616007_Commands(char* string);
void help(void);
...
//user cmd
                ...
                if (shell->line_position != 0) 
								{						
										MB1616007_Commands(&shell->line[0]);																
...
//finsh_run_line
//finsh_thread_entry
uint8_t read_out_data2(void)
{
	uint8_t t,out_data2=0;

	uint8_t byte=pca9539a_2.out_data;
	
	for(t=0;t<8;t++)	
	{
		out_data2+=((byte & 0x80)>>(7-t));
			byte<<=1;
	}	
	return out_data2;
}
...
uint8_t MB1616007_Commands(char* string)
{
	...
	if(!strncmp("fs.",string,3))
	{
		index=3;
		cmdtype=3;
		
	}
	finsh_run_line(&shell->parser, &shell->line[index]);
}

void readpin(int8_t indx)
{
	rt_kprintf("+ok@input.readpin(%02d,%d)\n",indx,inputGet(indx+1));
	
}
static char data_out[16]="0000000000000000";//data_out[0]=MSB=pca9539a_2

void readpca9539a_out(void)
{
		uint8_t i;
		
		for(i=0;i<8;i++)	
		{
			data_out[i+8]=(pca9539a_1.out_data & (1 << (7-i))) ? '1':'0';
			data_out[i]=(pca9539a_2.out_data & (1 << (i))) ? '1':'0';
		}
...
void readpins(void)
{
	if(cmdtype==1)	
	{
		...
		for(i=0;i<8;i++)
		{
			if(pca9539a_2.in_data & (1 << i))	data_read[7-i]='1'; else data_read[7-i]='0';
			if(pca9539a_1.in_data & (1 << i))	data_read[15-i]='1'; else data_read[15-i]='0';	
		}
		rt_kprintf("+ok@input.readpins(B%.16s)\n",data_read);
		//rt_kprintf("+ok@input.readpins(0x%08x)\n",(pca9539a_2.in_data)<<8 | pca9539a_1.in_data);
	}
	else if(cmdtype==2)	
	{
		readpca9539a_out();
		rt_kprintf("+ok@output.readpins(B%.16s)\n",data_out);	
	}
	
}
void writepin(uint8_t indx, uint8_t status)
{	
	outputSet(indx+1,status);
	readpca9539a_out();
	rt_kprintf("+ok@output.readpin(0x%08x)\n",read_out_data2()<<8 | pca9539a_1.out_data);
	//rt_kprintf("+ok@output.writepin(B%.16s)\n",data_out);
	//rt_kprintf("+ok@output.writepin(0x%08x)\n",(pca9539a_2.out_data)<<8 | pca9539a_1.out_data);
}
void writepins(uint32_t status)
{
	pca9539a_1.out_data=status;pca9539a_2.out_data=status>>4;
...
void writerecord(char *sting1, char *sting2,char *sting3, char *stingname,char *stingData)
{
	ef_set_env(stingname,stingData);
	ef_save_env();
	rt_kprintf("+ok@fs.writerecord()\n");
}
void readrecord(char *sting1, char *sting2,char *sting3,char *stingname)
{
	rt_kprintf("+ok@fs.readrecord(\"%s%s\")\n",stingname,ef_get_env(stingname));
}
//确定下fs.实际使用的命令格式
void help(void)
{
	...
	rt_kprintf("  input.readpins()\n");
	rt_kprintf("  input.readpin()\n");
	rt_kprintf("  output.writepin(indx, b)\n");
	rt_kprintf("  output.readpins()\n");
	rt_kprintf("  output.readpin()\n");
	rt_kprintf("  fs.readrecord(\"FileName\",\"Head\",\"Tail\",\"RecName\")\n");		
	rt_kprintf("  fs.writerecord(\"FileName\",\"Head\",\"Tail\",\"RecName\",\"RecContent\")\n");	
}
FINSH_FUNCTION_EXPORT(readpin,...);
FINSH_FUNCTION_EXPORT(readpins,...);
FINSH_FUNCTION_EXPORT(writepin,...);
FINSH_FUNCTION_EXPORT(help,...);
FINSH_FUNCTION_EXPORT(writerecord,...);
FINSH_FUNCTION_EXPORT(readrecord,...);
