...
//user
        if (ch < 0  ||  ch == '\r' )						//兼容\r\n项目
        {
            continue;
				...
				if (msh_is_used() == RT_FALSE)					//兼容Bx88项目  大小写不区分
				{
//user 大写-->小写
					if((g_tParam.Project_ID==BX88)&&(ch>='A')&& (ch<='Z'))
					{
						ch=ch+32;
					}											
				}
...
//user cmd
                ...
                if (shell->line_position != 0) 
								{						
									//rt_kprintf("\n");										
									//MakeLowercase(&shell->line[shell->line_curpos]);	
									//rt_kprintf("%s", &shell->line[shell->line_curpos]);
									if(ProcessCommand(&shell->line[0])==REPLY_OK) {;}	//rt_kprintf("\n");	
									else
									{
