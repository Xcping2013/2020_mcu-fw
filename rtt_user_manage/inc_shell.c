...
---finsh_thread_entry
		1. \r or \n	if (ch < 0)  -> if (ch < 0  ||  ch == '\r' )
		2. cmds_lowercase	 			 -> cmd lowercase or not
		3.	msh_exec 						 -> UserMsh_Commands
				finsh_run_line			 -> UserFinsh_Commands
		4. remove finsh return value
		   -finsh_run_line   finsh_stack_bottom
		
		5.msh.c 
			-cmd_mshMode_exit	cmd_mshMode_enter
		  -msh_help
*/
//user
...
void finsh_run_line(struct finsh_parser *parser, const char *line)
{
    ...
    if(shell->echo_mode)
//user comment 发送命令和返回命令有换行，不需要则注释掉
        rt_kprintf("\n");
        ...
        if (ch == '\r' || ch == '\n')
        {
            ...
            if (msh_is_used() == RT_TRUE)
            {
                if (shell->echo_mode)
                    rt_kprintf("\n");
//user cmd									
								UserMsh_Commands(shell->line, shell->line_position);
...
#endif
            {
                ...
                if (shell->line_position != 0) 
								{
//user cmd									
									UserFinsh_Commands(&shell->parser, shell->line,shell->line_position);
