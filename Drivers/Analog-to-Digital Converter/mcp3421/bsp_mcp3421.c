 ...
 */
//#include <rtthread.h>
float MCP3421GetValue(mcp3421_t *dev)
{
	float voltage = 0.0;
	//16BITS	
	u8 data[4];//,data2,regRead;
	do
	{
		delay_us(10);
		SoftI2c.reads(dev->pin,0,MCP3421_DEVICE_ADDRESS,0x00,&data[0],4);
	}
...
//	rt_kprintf("regSet=%d\r\n",MCP3421_set);
  //voltage = data.value * (lsb*1000 / gain);	
	...
	voltage = (int)((data[0]<<8)+data[1]) * (lsb*1000.0 / gain);	//mV
	
//	voltage = (u16)((data[0]<<8) | data[1]) * (lsb*1000*1000.0 / gain);	//mV
//	voltage = (u16)((data[0]<<8) | data[1]) * (62.5 / gain);	//mV

//	return (u16)((u16)data[0]<<8) + data[1]);	

//	rt_kprintf("regRead=%d\r\n",((u16)data[0]<<8 + data[1]));
	
//		rt_kprintf("data[0]=0x%x ,data[1]=0x%x,data[2]=0x%x\r\n",data[0] ,data[1],data[2]);
