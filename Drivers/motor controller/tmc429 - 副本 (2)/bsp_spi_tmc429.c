/*		采用结构体为函数参数的时候,FINSH就无法直接调用相关函数,所以未使用dev的独立文件需保留，用于调试和备份
TMC429_ParameterPresetToRam();
rt_hw_spi_tmc429_init()
Init429()
...
#include "bsp_spi_tmc429.h"

int tmc429_init(uint8_t Which429)
{
//	pinMode(dev.poscmp_pin, PIN_MODE_INPUT_PULLUP);
//  pinMode(dev.intout_pin, PIN_MODE_INPUT_PULLUP);	
		
	rt_kprintf("dev.spi_name=%s,dev.dev_name=%s\n", dev.spi_name,dev.dev_name);
...
********************************************************************/
UCHAR Read429Status(uint8_t Which429)
{
...
********************************************************************/
void ReadWrite429(uint8_t Which429, UCHAR *Read, UCHAR *Write)
{
...
********************************************************************/
void Write429Bytes(uint8_t Which429,UCHAR Address, UCHAR *Bytes)
{
...
********************************************************************/
void Write429Datagram(uint8_t Which429, UCHAR Address, UCHAR HighByte, UCHAR MidByte, UCHAR LowByte)
{
  UCHAR Write[4], Read[4];
...
********************************************************************/
void Write429Zero(uint8_t Which429, UCHAR Address)
{
  UCHAR Write[4], Read[4];
...
********************************************************************/
void Write429Short(uint8_t Which429, UCHAR Address, int Value)
{
  UCHAR Write[4], Read[4];
...
********************************************************************/
void Write429Int(uint8_t Which429, UCHAR Address, int Value)
{
  UCHAR Write[4], Read[4];
...
********************************************************************/
UCHAR Read429Bytes(uint8_t Which429, UCHAR Address, UCHAR *Bytes)
{
...
********************************************************************/
UCHAR Read429SingleByte(uint8_t Which429, UCHAR Address, UCHAR Index)
{
  UCHAR Read[4], Write[4];
...
********************************************************************/
int Read429Short(uint8_t Which429, UCHAR Address)
{
  UCHAR Read[4], Write[4];
  int Result;
...
/***************************************************************//**
   ...
   register value to 32 bit.
********************************************************************/
int Read429Int(uint8_t Which429, UCHAR Address)
{
  UCHAR Read[4], Write[4];
  int Result;
...
********************************************************************/
void Set429RampMode(uint8_t Which429, UCHAR Axis, UCHAR RampMode)
{
  UCHAR Write[4], Read[4];
...
/***************************************************************//**
   ...
   It is some TMC429 register bit twiddling.
********************************************************************/
void Set429SwitchMode(uint8_t Which429, UCHAR Axis, UCHAR SwitchMode)
{
  UCHAR Write[4], Read[4];
...
/***************************************************************//**
   ...
   values).
********************************************************************/
UCHAR SetAMax(uint8_t Which429, UCHAR Motor, UINT AMax)
{
  int pdiv, pmul, pm, pd;
  float p, p_reduced;
  int ramp_div;
...
/***************************************************************//**
   ...
   of that axis.
********************************************************************/
void HardStop(uint8_t Which429, UINT Motor)
{
...
void Init429(uint8_t Which429)
{
  ...
  for(Motor=0; Motor<3; Motor++)
  {
	...
	Write429Int(dev,IDX_IF_CONFIG_429, IFCONF_EN_SD|IFCONF_EN_REFR|IFCONF_SDO_INT|IFCONF_INV_DIR);

	Write429Datagram(dev,IDX_SMGP, 0x00, 0x04, 0x02);
  ...
  Write429Datagram(dev,IDX_REFCONF_RM|MOTOR0, 0x00, HARD_REF, 0x00);
  Write429Datagram(dev,IDX_REFCONF_RM|MOTOR1, 0x00, HARD_REF, 0x00);
  Write429Datagram(dev,IDX_REFCONF_RM|MOTOR2, 0x00, HARD_REF, 0x00);
